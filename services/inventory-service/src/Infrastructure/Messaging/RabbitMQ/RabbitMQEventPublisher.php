 ...
 * 
 * Implementação do EventPublisher usando RabbitMQ para o Inventory Service.
 */
final class RabbitMQEventPublisher implements EventPublisherInterface
{
    ...
    private function connect(): void
    {
        try {
            ...
            $this->logger->info('Inventory Service connected to RabbitMQ', [
                'host' => $this->host,
                'port' => $this->port,
                'vhost' => $this->vhost,
            ]);
        } catch (\Exception $e) {
            $this->logger->error('Inventory Service failed to connect to RabbitMQ', [
                'error' => $e->getMessage(),
            ]);
            throw $e;
        }
    ...
    public function publish(DomainEvent $event): void
    {
        try {
            ...
            $this->logger->info('Inventory Service event published to RabbitMQ', [
                'event' => $event->eventName(),
                'exchange' => $exchange,
                'routing_key' => $event->routingKey(),
            ]);
        } catch (\Exception $e) {
            $this->logger->error('Inventory Service failed to publish event to RabbitMQ', [
                'event' => $event->eventName(),
                'error' => $e->getMessage(),
            ]);
            throw $e;
    ...
    {
        // Extrai o prefixo do nome do evento (ex: "inventory" de "inventory.product.created")
